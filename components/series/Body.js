...
const Body = ({ series, changeCurrent, current }) => {
    return (
        <>
            {
                series && changeCurrent && current && 
                <div className="container lg:px-[2rem]  ">
                    <h1 className="text-xs lg:hidden uppercase bg-gray-700 text-center text-white px-5 py-2 lg:py-0"><a href="#all">View All from this series</a></h1>
                    <div className="flex flex-col justify-center !mt-3 !lg:mt-0 items-center space-y-3 mb-3">
                                ...
                                <h1 className="text-center  font-medium uppercase">{series.title}</h1>
                                {
                                    series.sermons.map((sermon, index) => (
                                        <div
                                            ...
                                            key={sermon._id} className={`${current._id === sermon._id ? "bg-primary-dark text-white" : "lg:hover:bg-gray-200 lg:hover:text-gray-800"} flex py-3 cursor-pointer  items-center justify-between space-x-2 px-2 border-b border-b-primary-black/10`}>
